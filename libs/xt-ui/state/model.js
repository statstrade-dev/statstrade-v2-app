...
function model(store,model_id){
  let current_value = page_core.model_ensure(store["node"],store["space_id"],store["group_id"],model_id);
  let [_group,current] = current_value;
  return current;
}
...
function subscribef(store,subscription_id,callback){
  let {group_id,node,space_id} = store;
