...
// statsui.basic.element.element-organisation/organisationCategories [20] 
...
// statsui.basic.element.element-organisation/OrganisationItem [32] 
  ...
  return (
    <T.XStack
      ...
      {isSelected ? (
        <ui.Tooltip content="Edit Organisation" placement="left">
          <T.Button
            ...
            pressStyle={{"backgroundColor":"#000","scale":0.9}}
            icon={Pencil}
            hoverStyle={{"backgroundColor":"#333","scale":1.1}}
            size="$5"
            position="absolute"
            backgroundColor="#000"
            left="-60px"/>
      ...
      <T.XStack
        ...
        flex={1}
        hoverStyle={{
            "borderColor":isSelected ? "#000" : "$color8",
            "cursor":"pointer"
          }}
        borderWidth={2}
        justifyContent="center"
...
// statsui.basic.element.element-organisation/OrganisationList [97] 
  ...
  return (
    <T.YStack gap="$4" width="100%" maxWidth={450} alignSelf="center">
      ...
      <T.XStack
        ...
        borderColor={isNewSelected ? "#000" : "$borderColor"}
        hoverStyle={{
            "borderColor":isNewSelected ? "#000" : "$color8",
            "cursor":"pointer"
          }}
        borderWidth={2}
        justifyContent="center"
        padding="$3"
        backgroundColor={isNewSelected ? "$color3" : "$color2"}
        minHeight={60}
        height={60}
        alignItems="center">
        <T.XStack gap="$3" alignItems="center">
          <T.Text
            fontWeight="600"
            fontSize="$4"
            color={isNewSelected ? "#000" : "$color8"}>{ui.t("Create New Organisation")}
          </T.Text>
        </T.XStack>
      </T.XStack>
...
// statsui.basic.element.element-organisation/OrganisationCategories [158] 
export function OrganisationCategories({tags,toggleTag}){
  return (
    <T.YStack gap="$3">
      <T.Text
        fontSize="$3"
        color="$color11"
        fontWeight="600"
        textTransform="uppercase">{ui.t("Tags")}
      </T.Text>
      <T.XStack flexWrap="wrap" gap="$2" width="100%">
        {organisationCategories.map(function (item){
          let {id,label,icon} = item;
          let isSelected = tags.has(id);
          return (
            <T.Button
              animation="quick"
              ...
              borderRadius="$3"
              scale={isSelected ? 1.02 : 1}
              borderColor={isSelected ? "#000" : "$color4"}
              flexBasis="45%"
              icon={icon}
              hoverStyle={{
                  "borderColor":isSelected ? "#000" : "$color6",
                  "backgroundColor":isSelected ? "#333" : "$color3"
                }}
              borderWidth={1}
              size="$4"
              backgroundColor={isSelected ? "#000" : "$color2"}
              flexGrow={1}>{label}
            </T.Button>);
        })}
      </T.XStack>
...
// statsui.basic.element.element-organisation/OrganisationForm [193] 
    ...
    <T.Form
      ...
      <T.YStack marginTop="$3" gap="$6" marginHorizontal="$2">
        ...
        <ui_form.FormInput
          ...
          numberOfLines={4}
          disabled={isBusy}/>
        <OrganisationCategories tags={tags} toggleTag={toggleTag}/>
