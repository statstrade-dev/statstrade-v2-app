...
function kitchen_sink_bindings(){
  return {
    "name":{
        "source":"state",
        "space_id":"app",
    ...
    "orgs":{"source":"state","space_id":"app","path":["orgs"],"default":[]},
    "pending":{
        "source":"state",
        "space_id":"app",
        "path":["pending"],
...
function install_handlers(node){
  base_util.register_handler(node,"demo/set-name",function (space,args,request,node){
    return view.state_set(
      node,
      request["space"],
      "demo/kitchen-sink",
      ["form","name"],
  ...
  base_util.register_handler(node,"demo/set-about",function (space,args,request,node){
    return view.state_set(
      node,
      request["space"],
      "demo/kitchen-sink",
  ...
  base_util.register_handler(node,"demo/reset-form",function (space,args,request,node){
    let space_id = request["space"];
    view.state_set(node,space_id,"demo/kitchen-sink",["form","name"],"");
    view.state_set(node,space_id,"demo/kitchen-sink",["form","about"],"");
    return true;
  },null);
  base_util.register_handler(node,"demo/create-org",function (space,args,request,node){
    let space_id = request["space"];
    let name = view.state_get(node,space_id,"demo/kitchen-sink",["form","name"],"");
    let about = view.state_get(node,space_id,"demo/kitchen-sink",["form","about"],"");
    if(0 < name.length){
      let orgs = view.state_get(node,space_id,"demo/kitchen-sink",["orgs"],[]);
      view.state_set(
        node,
        space_id,
        "demo/kitchen-sink",
        ["orgs"],
        xtd.arr_concat(orgs,[{"name":name,"about":about}])
      );
      view.state_set(node,space_id,"demo/kitchen-sink",["form","name"],"");
      view.state_set(node,space_id,"demo/kitchen-sink",["form","about"],"");
    }
...
function kitchen_sink_render(snapshot){
  ...
  let about = snapshot["about"] || "";
  let orgs = snapshot["orgs"] || [];
  ...
  return view.node("ui/column",{"class":"flex flex-col gap-8 p-8 max-w-3xl"},[
    ...
    sink_section("Form",[
      view.node("ui/card",{"class":"flex flex-col gap-4 p-6"},[
        ...
        view.node("ui/card-content",{"class":"flex flex-col gap-4 p-0"},[
          ...
          view.node("ui/description",{
            "value":(0 < name.length) ? ("Hello, " + name + "! state-set roundtrip via xt.substrate.") : "Type a name to see the live substrate roundtrip."
          },[])
        ]),
        view.node("ui/card-footer",{"class":"flex flex-row justify-end gap-2 p-0"},[
          view.node("ui/button",{
            "variant":"ghost",
            "on_press":view.action("demo/reset-form",null)
          },["Cancel"]),
          view.node("ui/button",{
            "pending":pending,
      ...
      view.node("ui/column",{"class":"flex flex-col gap-2"},xtd.arr_map(orgs,function (org){
        return view.node("ui/alert",{"variant":"default"},[
          view.node("ui/text",{"value":org["name"] + " - " + org["about"]},[])
        ]);
      }))
    ]),
    sink_section("Image",[
    ...
    sink_section("Table",[
      view.node("ui/table",{"class":"w-full text-sm"},[
        ...
        view.node("ui/table-body",{},xtd.arr_map(fills,function (fill){
...
module.exports = {
  ...
  ["has_valp"]:has_valp,
  ["kitchen_sink_bindings"]:kitchen_sink_bindings,
